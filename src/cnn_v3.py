...
# 定义残差块
def residual_block(x, filters, kernel_size=3, stride=1, pool_size=2):
    shortcut = x
...
def create_multi_input_model(input_shape_time, num_handcrafted, num_classes):
    ...
    x = residual_block(time_input, filters=64, stride=1, pool_size=2)
    x = residual_block(x, filters=128, stride=1, pool_size=2)
    x = residual_block(x, filters=256, stride=1, pool_size=2)
