...
model_cnn_lstm = create_lstm(input_shape, num_classes)
model_cnn_lstm.summary()


# 训练配置
def compile_and_train(model, X_train, y_train, X_val, y_val, model_name, epochs=200, batch_size=128):
    model.compile(
        optimizer='adam',
...
# 训练
print("\n训练 LSTM ...")
history_cnn_lstm = compile_and_train(model_cnn_lstm, X_train, y_train_enc, X_val, y_val_enc, 'lstm')
...
y_pred_cnn_lstm = evaluate_model(model_cnn_lstm, X_test, y_test_enc, class_names, 'LSTM')
...
plot_training_history(history_cnn_lstm, 'LSTM')