...
print(f"\n输入形状: {input_shape}")
model_cnn_lstm = create_cnn_lstm(input_shape, num_classes)
model_cnn_lstm.summary()


# 训练配置
def compile_and_train(model, X_train, y_train, X_val, y_val, model_name, epochs=100, batch_size=64):
    model.compile(
        optimizer='adam',
...
# 训练
print("\n训练 CNN+LSTM ...")
history_cnn_lstm = compile_and_train(model_cnn_lstm, X_train, y_train_enc, X_val, y_val_enc, 'cnn_lstm')
...
y_pred_cnn_lstm = evaluate_model(model_cnn_lstm, X_test, y_test_enc, class_names, 'CNN+LSTM')
